    ...
    public class SimpleKinematicMover : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            if (isMoving)
            {
                ...
                {
                    UnityEngine.Debug.Log("moving to " + targetPosition.ToString());
        ...
        bool _canMoveTo(Vector3 target, float colliderPadding = 0.005f, LayerMask movementBlockerLayers = default)
        {
            ...
            Collider[] allHits = UnityEngine.Physics.OverlapBox(col.center + target,
                colliderHalfExtents,
                rot,
            ...
            foreach (var collider in allHits)
            {
                ...
                {
                    UnityEngine.Debug.Log("movement blocked by" + collider.name);
                    return false;
